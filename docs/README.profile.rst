...
* ``cowspace_size``: The size of cowspace, which is the size of root in live environment. You can use a prefix such as M or G after the number.
...
* ``%COWSPACE_SIZE%``: Set this using the ``cowspace_size`` in ``profiledef.sh``.
